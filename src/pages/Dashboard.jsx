  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: 'linear-gradient(135deg,#07060f 0%,#0d0b1a 30%,#0a0f1e 60%,#0d0a18 100%)', position: 'relative', overflow: 'hidden' }}>
      ...
      <style>{`
        ...
        .task-card:hover { transform:translateY(-5px) !important; box-shadow:0 16px 40px rgba(0,0,0,0.4) !important; }
        ...
        @media(max-width:900px) {
          ...
          .stats-db  { grid-template-columns:repeat(2,1fr) !important; gap:10px !important; }
          .cards-db  { grid-template-columns:1fr 1fr !important; gap:12px !important; }
          .banner-db { flex-direction:column !important; align-items:flex-start !important; gap:12px !important; }
          .banner-r  { display:grid !important; grid-template-columns:repeat(2,1fr) !important; width:100% !important; gap:8px !important; }
          .greet-row { flex-direction:column !important; align-items:flex-start !important; gap:12px !important; }
          .top-btns  { width:100% !important; justify-content:space-between !important; }
        }
        @media(max-width:560px) {
          .cards-db  { grid-template-columns:1fr !important; }
          .stats-db  { grid-template-columns:repeat(2,1fr) !important; }
          .cats-db   { overflow-x:auto !important; flex-wrap:nowrap !important; padding-bottom:6px; -webkit-overflow-scrolling:touch; }
          .banner-r  { grid-template-columns:repeat(2,1fr) !important; }
          .greet-row { gap:10px !important; }
          .top-btns  { gap:8px !important; }
          .add-btn   { flex:1 !important; justify-content:center !important; }
        }
      `}</style>
      ...
      <main className="main-db" style={{ marginLeft: '268px', padding: '28px', flex: 1, minHeight: '100vh', position: 'relative', zIndex: 1 }}>
        ...
        {/* ── GREETING ── */}
        <div className="greet-row" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px', gap: '12px', animation: 'fadeInUp 0.4s ease' }}>
          <div>
            <h1 style={{ fontSize: '24px', fontWeight: '800', color: '#fff', margin: 0, letterSpacing: '-0.5px' }}>
              {greetEmoji} {greeting}, {user?.name?.split(' ')[0] || 'User'}!
            </h1>
            <p style={{ color: '#475569', marginTop: '4px', fontSize: '12px' }}>{today}</p>
          </div>
          <div className="top-btns" style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
            {/* View toggle */}
            <div style={{ display: 'flex', background: 'rgba(255,255,255,0.05)', borderRadius: '10px', padding: '4px', border: '1px solid rgba(255,255,255,0.06)', flexShrink: 0 }}>
              {[{ mode: 'grid', icon: '⊞' },{ mode: 'list', icon: '☰' }].map(v => (
                <button key={v.mode} onClick={() => setViewMode(v.mode)} style={{ background: viewMode===v.mode ? '#4f46e5' : 'transparent', color: viewMode===v.mode ? '#fff' : '#64748b', border: 'none', borderRadius: '7px', padding: '6px 11px', cursor: 'pointer', fontSize: '15px', transition: 'all 0.2s' }}>{v.icon}</button>
              ))}
            </div>
            {/* Add button */}
            <button className="add-btn" onClick={() => navigate('/add')} style={{ background: 'linear-gradient(135deg,#4f46e5,#7c3aed)', color: '#fff', border: 'none', borderRadius: '12px', padding: '10px 18px', fontSize: '14px', fontWeight: '700', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px', transition: 'all 0.2s', boxShadow: '0 4px 16px rgba(79,70,229,0.4)', whiteSpace: 'nowrap' }}>
              <span>➕</span>
        ...
        {/* ── USER BANNER ── */}
        <div className="banner-db" style={{ display: 'flex', alignItems: 'center', gap: '16px', background: 'rgba(255,255,255,0.03)', borderRadius: '20px', padding: '16px 20px', marginBottom: '20px', border: '1px solid rgba(255,255,255,0.06)', boxShadow: '0 8px 32px rgba(0,0,0,0.25)', animation: 'fadeInUp 0.5s ease', position: 'relative', overflow: 'hidden', backdropFilter: 'blur(10px)' }}>
          <div style={{ position: 'absolute', top: '-20px', right: '-20px', width: '100px', height: '100px', background: 'radial-gradient(circle,rgba(79,70,229,0.15),transparent)', borderRadius: '50%', pointerEvents: 'none' }} />
          <div style={{ width: '50px', height: '50px', borderRadius: '50%', background: 'linear-gradient(135deg,#4f46e5,#7c3aed)', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: '800', fontSize: '18px', flexShrink: 0, boxShadow: '0 6px 20px rgba(79,70,229,0.5)' }}>
            {getInitials(user?.name)}
          </div>
          <div style={{ flex: 1, minWidth: 0 }}>
            <p style={{ color: '#fff', fontSize: '16px', fontWeight: '800', margin: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{user?.name}</p>
            <p style={{ color: '#475569', fontSize: '12px', marginTop: '2px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>📧 {user?.email}</p>
          </div>
          <div className="banner-r" style={{ display: 'flex', alignItems: 'center', gap: '16px', flexShrink: 0 }}>
            {[
              ...
              <div key={i} style={{ textAlign: 'center', background: 'rgba(255,255,255,0.04)', borderRadius: '10px', padding: '8px 12px', border: '1px solid rgba(255,255,255,0.05)' }}>
                <p style={{ color: s.color, fontSize: '15px', fontWeight: '800', margin: 0 }}>{s.val}</p>
        ...
        {/* ── STATS ── */}
        <div className="stats-db" style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '14px', marginBottom: '20px' }}>
          {[
            ...
            <div key={i} className="stat-card" style={{ background: s.grad, borderRadius: '16px', padding: '16px', display: 'flex', flexDirection: 'column', gap: '10px', border: `1px solid ${s.color}30`, boxShadow: '0 6px 20px rgba(0,0,0,0.25),inset 0 1px 0 rgba(255,255,255,0.05)', transition: 'transform 0.2s', animation: `fadeInUp 0.4s ease ${i*0.08}s both` }}>
              <div style={{ width: '40px', height: '40px', borderRadius: '12px', background: s.color+'25', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '18px', boxShadow: `0 4px 12px ${s.color}30` }}>
                {s.icon}
              </div>
              <div>
                <p style={{ color: s.color, fontSize: '26px', fontWeight: '800', margin: 0, lineHeight: 1 }}>{s.value}</p>
                <p style={{ color: '#64748b', fontSize: '11px', margin: '4px 0 0', fontWeight: '500' }}>{s.label}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        {schedules.length > 0 && (
          <div className="cats-db" style={{ display: 'flex', gap: '8px', marginBottom: '16px', flexWrap: 'wrap' }}>
            {categories.map(cat => (
              <button key={cat} onClick={() => setFilterCat(cat)} style={{ padding: '7px 14px', borderRadius: '20px', fontSize: '12px', fontWeight: '600', cursor: 'pointer', transition: 'all 0.2s', border: `1px solid ${filterCat===cat ? '#4f46e5' : 'rgba(255,255,255,0.08)'}`, background: filterCat===cat ? 'linear-gradient(135deg,#4f46e5,#7c3aed)' : 'rgba(255,255,255,0.04)', color: filterCat===cat ? '#fff' : '#64748b', whiteSpace: 'nowrap', boxShadow: filterCat===cat ? '0 4px 12px rgba(79,70,229,0.35)' : 'none', flexShrink: 0 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px', flexWrap: 'wrap', gap: '8px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            <h2 style={{ fontSize: '16px', fontWeight: '800', color: '#fff', margin: 0 }}>📋 My Schedules</h2>
        ...
        {/* ── EMPTY STATE ── */}
        {filtered.length === 0 && (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '48px 20px', background: 'rgba(255,255,255,0.02)', borderRadius: '20px', border: '2px dashed rgba(255,255,255,0.07)', animation: 'fadeInUp 0.5s ease', backdropFilter: 'blur(10px)' }}>
            <div style={{ width: '90px', height: '90px', borderRadius: '50%', background: 'rgba(79,70,229,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '16px', border: '2px solid rgba(79,70,229,0.2)', fontSize: '44px' }}>
              🗓️
            </div>
            <h3 style={{ color: '#fff', fontSize: '20px', fontWeight: '800', margin: '0 0 8px', textAlign: 'center' }}>No schedules yet!</h3>
            <p style={{ color: '#64748b', fontSize: '14px', marginBottom: '24px', textAlign: 'center', maxWidth: '280px' }}>
            ...
            <div style={{ display: 'flex', gap: '10px', marginBottom: '24px', flexWrap: 'wrap', justifyContent: 'center' }}>
              {[
                ...
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '8px', background: 'rgba(255,255,255,0.04)', borderRadius: '10px', padding: '8px 14px', border: '1px solid rgba(255,255,255,0.06)' }}>
                  <span style={{ fontSize: '16px' }}>{tip.icon}</span>
                  <span style={{ color: '#64748b', fontSize: '12px' }}>{tip.text}</span>
                </div>
              ))}
            </div>
            <button onClick={() => navigate('/add')} style={{ background: 'linear-gradient(135deg,#4f46e5,#7c3aed)', color: '#fff', border: 'none', borderRadius: '12px', padding: '13px 28px', fontSize: '14px', fontWeight: '700', cursor: 'pointer', boxShadow: '0 6px 24px rgba(79,70,229,0.4)', display: 'flex', alignItems: 'center', gap: '8px' }}>
        ...
        {filtered.length > 0 && viewMode === 'grid' && (
          <div className="cards-db" style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '16px' }}>
            {filtered.map((item, idx) => {
              ...
              return (
                <div
                  key={item.id}
                  className="task-card"
                  style={{ borderRadius: '18px', padding: 0, border: `1px solid ${item.color}40`, position: 'relative', overflow: 'hidden', display: 'flex', flexDirection: 'column', cursor: 'pointer', animation: isNew ? 'popIn 0.65s cubic-bezier(0.34,1.56,0.64,1) forwards' : `fadeInUp 0.4s ease ${idx*0.06}s both`, transition: 'transform 0.25s,box-shadow 0.25s', boxShadow: `0 4px 20px rgba(0,0,0,0.3),inset 0 1px 0 rgba(255,255,255,0.04)`, background: item.bg, backdropFilter: 'blur(10px)' }}
                >
                  {/* Top accent bar */}
                  <div style={{ height: '3px', background: `linear-gradient(90deg,${item.color},${item.color}66)`, flexShrink: 0 }} />

                  <div style={{ padding: '14px 16px', flex: 1, display: 'flex', flexDirection: 'column', gap: '10px' }}>
                    {/* Top row */}
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '7px' }}>
                        <div style={{ width: '30px', height: '30px', borderRadius: '9px', background: item.color+'20', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '14px', flexShrink: 0 }}>
                          {item.icon}
                        </div>
                        <span style={{ background: item.color+'20', color: item.color, padding: '2px 9px', borderRadius: '20px', fontSize: '10px', fontWeight: '700' }}>
                          {item.category}
                        </span>
                      </div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
                        {isNew && <span style={{ background: 'linear-gradient(135deg,#10b981,#059669)', color: '#fff', borderRadius: '20px', padding: '2px 7px', fontSize: '9px', fontWeight: '700' }}>✨ New</span>}
                        <div style={{ width: '9px', height: '9px', borderRadius: '50%', background: item.priority==='High' ? '#f43f5e' : item.priority==='Medium' ? '#f59e0b' : '#10b981', boxShadow: `0 0 6px ${item.priority==='High' ? '#f43f5e' : item.priority==='Medium' ? '#f59e0b' : '#10b981'}` }} />
                      </div>
                    </div>

                    {/* Title */}
                    <h3 style={{ fontSize: '14px', fontWeight: '700', color: '#fff', lineHeight: '1.4', margin: 0 }}>{item.title}</h3>

                    {/* Description */}
                    {item.description && (
                      <p style={{ fontSize: '11px', color: '#64748b', margin: 0, lineHeight: '1.5', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
                        {item.description}
                      </p>
                    )}
                    ...
                    {/* Footer */}
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '4px', paddingBottom: '2px' }}>
                      <span style={{ color: '#64748b', fontSize: '11px' }}>📅 {item.date}</span>
                      <span style={{ background: item.color+'25', color: item.color, padding: '3px 8px', borderRadius: '7px', fontSize: '10px', fontWeight: '600' }}>
                        ⏰ {item.time}
                      </span>
                    </div>
                  </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {filtered.map((item, idx) => {
              ...
              return (
                <div
                  key={item.id}
                  style={{ display: 'flex', alignItems: 'center', gap: '12px', background: 'rgba(255,255,255,0.03)', borderRadius: '14px', padding: '12px 16px', border: '1px solid rgba(255,255,255,0.06)', borderLeft: `4px solid ${item.color}`, boxShadow: '0 4px 16px rgba(0,0,0,0.2)', animation: isNew ? 'popIn 0.6s ease forwards' : `fadeInUp 0.3s ease ${idx*0.05}s both`, transition: 'all 0.2s', backdropFilter: 'blur(10px)', flexWrap: 'wrap' }}
                >
                  <div style={{ width: '38px', height: '38px', borderRadius: '10px', background: item.color+'20', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '16px', flexShrink: 0 }}>
                    {item.icon}
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <p style={{ color: '#fff', fontWeight: '700', fontSize: '13px', margin: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{item.title}</p>
                    <p style={{ color: '#64748b', fontSize: '11px', margin: '3px 0 0' }}>📅 {item.date} · ⏰ {item.time}</p>
                  </div>
                  <span style={{ background: item.color+'20', color: item.color, padding: '3px 10px', borderRadius: '20px', fontSize: '11px', fontWeight: '700', flexShrink: 0 }}>
                    {item.category}
                  </span>
                  <div style={{ width: '9px', height: '9px', borderRadius: '50%', background: item.priority==='High' ? '#f43f5e' : item.priority==='Medium' ? '#f59e0b' : '#10b981', flexShrink: 0, boxShadow: `0 0 5px ${item.priority==='High' ? '#f43f5e' : item.priority==='Medium' ? '#f59e0b' : '#10b981'}` }} />
                </div>
              )
            })}
          </div>
